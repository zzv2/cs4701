...
def backPropagation(layers, decision, learnRate, tolerance, numSamples):
	#do shit here
	size = len(layers.keys())
	output_neuron = layers["layer" + str(size-1)][0]
	output_neuron.error= (2*(output_neuron.output - decision)*dsigmoidDX(output_neuron.weightedSum))
	...
	for j in range(2, size):
		...
		for k in range(0, len(currentLayer)):
			sigPrime = dsigmoidDX(currentLayer[i].weightedSum)
...
def forwardPropagation(layers):
	...
	for x in range(1, size):
		...
		for y in range(0, len(currentLayer)):
			...
			currentLayer[y].output = sigmoid(currentLayer[y].weightedSum)
			# print("currinW:")
...
def test(layers, rowlist, numSamples):
	numInputs = 4
	neuronList = []
...
def train(network, dataFile, learnRate, epochs, tolerance, numSamples):
	layers = network
	...
	with open(dataFile, "r") as csvfile:
		...
		for i in range(0, epochs):
			...
			for row in rowlist:
				...
				layers = backPropagation(layers, decision, learnRate, tolerance, numSamples)
				k+=1
				if k >=numSamples:#
					size = len(layers.keys())
					for x in range(1, size):
		...
		test(layers, rowlist, numSamples)
...
def main():
	...
	train(layers, "trainData.csv", 0.01, 300, 1e-5, 4000)
