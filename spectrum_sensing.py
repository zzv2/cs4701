# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'spectrum_sensing.ui'
#
# Created by: PyQt4 UI code generator 4.11.4
#
# WARNING! All changes made in this file will be lost!

from PyQt4 import QtCore, QtGui

try:
    _fromUtf8 = QtCore.QString.fromUtf8
except AttributeError:
    def _fromUtf8(s):
        return s

try:
    _encoding = QtGui.QApplication.UnicodeUTF8
    def _translate(context, text, disambig):
        return QtGui.QApplication.translate(context, text, disambig, _encoding)
except AttributeError:
    def _translate(context, text, disambig):
        return QtGui.QApplication.translate(context, text, disambig)

class Ui_SpectrumSensingNN(object):
    def setupUi(self, SpectrumSensingNN):
        SpectrumSensingNN.setObjectName(_fromUtf8("SpectrumSensingNN"))
        SpectrumSensingNN.resize(782, 897)
        SpectrumSensingNN.setAccessibleName(_fromUtf8(""))
        self.centralwidget = QtGui.QWidget(SpectrumSensingNN)
        self.centralwidget.setObjectName(_fromUtf8("centralwidget"))
        self.verticalLayout = QtGui.QVBoxLayout(self.centralwidget)
        self.verticalLayout.setObjectName(_fromUtf8("verticalLayout"))
        self.run = QtGui.QGroupBox(self.centralwidget)
        self.run.setTitle(_fromUtf8(""))
        self.run.setObjectName(_fromUtf8("run"))
        self.verticalLayout_2 = QtGui.QVBoxLayout(self.run)
        self.verticalLayout_2.setObjectName(_fromUtf8("verticalLayout_2"))
        self.network_params = QtGui.QGroupBox(self.run)
        self.network_params.setEnabled(True)
        self.network_params.setObjectName(_fromUtf8("network_params"))
        self.gridLayout = QtGui.QGridLayout(self.network_params)
        self.gridLayout.setObjectName(_fromUtf8("gridLayout"))
        self.label_2 = QtGui.QLabel(self.network_params)
        self.label_2.setObjectName(_fromUtf8("label_2"))
        self.gridLayout.addWidget(self.label_2, 1, 0, 1, 1)
        self.neurons_h1 = QtGui.QSpinBox(self.network_params)
        self.neurons_h1.setMinimum(1)
        self.neurons_h1.setProperty("value", 5)
        self.neurons_h1.setObjectName(_fromUtf8("neurons_h1"))
        self.gridLayout.addWidget(self.neurons_h1, 0, 1, 1, 1)
        self.neurons_h2 = QtGui.QSpinBox(self.network_params)
        self.neurons_h2.setProperty("value", 0)
        self.neurons_h2.setObjectName(_fromUtf8("neurons_h2"))
        self.gridLayout.addWidget(self.neurons_h2, 1, 1, 1, 1)
        self.label = QtGui.QLabel(self.network_params)
        self.label.setObjectName(_fromUtf8("label"))
        self.gridLayout.addWidget(self.label, 0, 0, 1, 1)
        self.verticalLayout_2.addWidget(self.network_params)
        self.training_params = QtGui.QGroupBox(self.run)
        self.training_params.setObjectName(_fromUtf8("training_params"))
        self.gridLayout_3 = QtGui.QGridLayout(self.training_params)
        self.gridLayout_3.setObjectName(_fromUtf8("gridLayout_3"))
        self.num_epochs = QtGui.QSpinBox(self.training_params)
        self.num_epochs.setMaximum(1000000)
        self.num_epochs.setSingleStep(100)
        self.num_epochs.setProperty("value", 300)
        self.num_epochs.setObjectName(_fromUtf8("num_epochs"))
        self.gridLayout_3.addWidget(self.num_epochs, 0, 1, 1, 1)
        self.label_11 = QtGui.QLabel(self.training_params)
        self.label_11.setObjectName(_fromUtf8("label_11"))
        self.gridLayout_3.addWidget(self.label_11, 0, 0, 1, 1)
        self.num_samples = QtGui.QSpinBox(self.training_params)
        self.num_samples.setMinimum(100)
        self.num_samples.setMaximum(100000)
        self.num_samples.setSingleStep(1000)
        self.num_samples.setProperty("value", 2000)
        self.num_samples.setObjectName(_fromUtf8("num_samples"))
        self.gridLayout_3.addWidget(self.num_samples, 5, 1, 1, 1)
        self.label_3 = QtGui.QLabel(self.training_params)
        self.label_3.setObjectName(_fromUtf8("label_3"))
        self.gridLayout_3.addWidget(self.label_3, 2, 0, 1, 1)
        self.training_tolerance = QtGui.QDoubleSpinBox(self.training_params)
        self.training_tolerance.setDecimals(7)
        self.training_tolerance.setMaximum(1.0)
        self.training_tolerance.setSingleStep(1e-06)
        self.training_tolerance.setProperty("value", 0.3)
        self.training_tolerance.setObjectName(_fromUtf8("training_tolerance"))
        self.gridLayout_3.addWidget(self.training_tolerance, 4, 1, 1, 1)
        self.label_4 = QtGui.QLabel(self.training_params)
        self.label_4.setObjectName(_fromUtf8("label_4"))
        self.gridLayout_3.addWidget(self.label_4, 4, 0, 1, 1)
        self.label_6 = QtGui.QLabel(self.training_params)
        self.label_6.setObjectName(_fromUtf8("label_6"))
        self.gridLayout_3.addWidget(self.label_6, 5, 0, 1, 1)
        self.learning_rate = QtGui.QDoubleSpinBox(self.training_params)
        self.learning_rate.setDecimals(4)
        self.learning_rate.setMaximum(1.0)
        self.learning_rate.setSingleStep(0.01)
        self.learning_rate.setProperty("value", 0.01)
        self.learning_rate.setObjectName(_fromUtf8("learning_rate"))
        self.gridLayout_3.addWidget(self.learning_rate, 2, 1, 1, 1)
        self.batch_size = QtGui.QSpinBox(self.training_params)
        self.batch_size.setMinimum(10)
        self.batch_size.setMaximum(100000)
        self.batch_size.setSingleStep(100)
        self.batch_size.setProperty("value", 200)
        self.batch_size.setObjectName(_fromUtf8("batch_size"))
        self.gridLayout_3.addWidget(self.batch_size, 6, 1, 1, 1)
        self.label_7 = QtGui.QLabel(self.training_params)
        self.label_7.setObjectName(_fromUtf8("label_7"))
        self.gridLayout_3.addWidget(self.label_7, 6, 0, 1, 1)
        self.label_5 = QtGui.QLabel(self.training_params)
        self.label_5.setObjectName(_fromUtf8("label_5"))
        self.gridLayout_3.addWidget(self.label_5, 7, 0, 1, 1)
        self.test_samples = QtGui.QSpinBox(self.training_params)
        self.test_samples.setMinimum(10)
        self.test_samples.setMaximum(10000)
        self.test_samples.setSingleStep(100)
        self.test_samples.setProperty("value", 500)
        self.test_samples.setObjectName(_fromUtf8("test_samples"))
        self.gridLayout_3.addWidget(self.test_samples, 7, 1, 1, 1)
        self.verticalLayout_2.addWidget(self.training_params)
        self.train_button = QtGui.QPushButton(self.run)
        self.train_button.setObjectName(_fromUtf8("train_button"))
        self.verticalLayout_2.addWidget(self.train_button)
        self.verticalLayout.addWidget(self.run)
        self.learning_curve = QtGui.QGroupBox(self.centralwidget)
        self.learning_curve.setObjectName(_fromUtf8("learning_curve"))
        self.verticalLayout_3 = QtGui.QVBoxLayout(self.learning_curve)
        self.verticalLayout_3.setObjectName(_fromUtf8("verticalLayout_3"))
        self.epoch_loss_plot = MatplotlibWidget(self.learning_curve)
        self.epoch_loss_plot.setMinimumSize(QtCore.QSize(700, 400))
        self.epoch_loss_plot.setMaximumSize(QtCore.QSize(1000, 1000))
        self.epoch_loss_plot.setObjectName(_fromUtf8("epoch_loss_plot"))
        self.verticalLayout_3.addWidget(self.epoch_loss_plot)
        self.verticalLayout.addWidget(self.learning_curve)
        self.copyright = QtGui.QLabel(self.centralwidget)
        self.copyright.setAlignment(QtCore.Qt.AlignCenter)
        self.copyright.setObjectName(_fromUtf8("copyright"))
        self.verticalLayout.addWidget(self.copyright)
        SpectrumSensingNN.setCentralWidget(self.centralwidget)
        self.menubar = QtGui.QMenuBar(SpectrumSensingNN)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 782, 21))
        self.menubar.setObjectName(_fromUtf8("menubar"))
        SpectrumSensingNN.setMenuBar(self.menubar)
        self.statusbar = QtGui.QStatusBar(SpectrumSensingNN)
        self.statusbar.setObjectName(_fromUtf8("statusbar"))
        SpectrumSensingNN.setStatusBar(self.statusbar)

        self.retranslateUi(SpectrumSensingNN)
        QtCore.QMetaObject.connectSlotsByName(SpectrumSensingNN)
        SpectrumSensingNN.setTabOrder(self.neurons_h1, self.neurons_h2)
        SpectrumSensingNN.setTabOrder(self.neurons_h2, self.num_epochs)
        SpectrumSensingNN.setTabOrder(self.num_epochs, self.learning_rate)
        SpectrumSensingNN.setTabOrder(self.learning_rate, self.training_tolerance)
        SpectrumSensingNN.setTabOrder(self.training_tolerance, self.num_samples)
        SpectrumSensingNN.setTabOrder(self.num_samples, self.batch_size)
        SpectrumSensingNN.setTabOrder(self.batch_size, self.train_button)

    def retranslateUi(self, SpectrumSensingNN):
        SpectrumSensingNN.setWindowTitle(_translate("SpectrumSensingNN", "SpectrumSensingGUI", None))
        self.network_params.setTitle(_translate("SpectrumSensingNN", "Network Params", None))
        self.label_2.setText(_translate("SpectrumSensingNN", "Number of neurons in\n"
"Hidden Layer 2", None))
        self.label.setText(_translate("SpectrumSensingNN", "Number of neurons in\n"
"Hidden Layer 1", None))
        self.training_params.setTitle(_translate("SpectrumSensingNN", "Training Params", None))
        self.label_11.setText(_translate("SpectrumSensingNN", "Num Epochs", None))
        self.label_3.setText(_translate("SpectrumSensingNN", "Learning Rate", None))
        self.label_4.setText(_translate("SpectrumSensingNN", "Training tolerance", None))
        self.label_6.setText(_translate("SpectrumSensingNN", "Number of Samples", None))
        self.label_7.setText(_translate("SpectrumSensingNN", "Batch Size", None))
        self.label_5.setText(_translate("SpectrumSensingNN", "Test Samples", None))
        self.train_button.setText(_translate("SpectrumSensingNN", "Train", None))
        self.learning_curve.setTitle(_translate("SpectrumSensingNN", "Results", None))
        self.copyright.setText(_translate("SpectrumSensingNN", "Copyright 2016 Zach Zweig-Vinegar, Nick Polosky", None))

from matplotlibwidget import MatplotlibWidget
